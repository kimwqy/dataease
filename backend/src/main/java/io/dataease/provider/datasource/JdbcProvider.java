...
public class JdbcProvider extends DefaultJdbcProvider {
    ...
    public String getTablesSql(DatasourceRequest datasourceRequest) throws Exception {
        ...
        switch (datasourceType) {
            ...
            case xugu:
                XuGuConfiguration xuGuConfiguration = new Gson().fromJson(datasourceRequest.getDatasource().getConfiguration(), XuGuConfiguration.class);
                return String.format("select TABLE_NAME,COMMENTS FROM all_tables WHERE user_id IN (SELECT user_id FROM all_users WHERE user_name = '%s');",xuGuConfiguration.getUsername().toUpperCase());
            case engine_doris:
            case ds_doris:
            case StarRocks:
            case hive:
